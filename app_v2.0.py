...
<body>
  <div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <div class="d-flex gap-2">
      ...
      <a class="btn btn-sm btn-info" href="/basket_locations">Basket Locations</a>
...
HTML_BASKET_LOCATIONS = """
<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Basket Locations</title>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css">
  <style>
    body { padding: 20px; }
    table thead th { text-align: left !important; }
...
<body>
  <div class="d-flex align-items-center justify-content-between mb-3">
    <h1 class="h4 mb-0">Basket Locations</h1>
    <div class="d-flex gap-2">
      <a class="btn btn-sm btn-secondary" href="/">← Back</a>
      <a class="btn btn-sm btn-success" href="/track">Track Item → Basket</a>
      <a class="btn btn-sm btn-warning" href="/detector">Run Detector</a>
  ...
  {% else %}
    ...
    <small class="text-muted d-block mt-2">
      显示 basket_locations 表中记录的所有篮子位置（gx, gy 采用龙门坐标系，时间为本地时间）。
    </small>
  {% endif %}
</body>
</html>
...
@app.route("/basket_locations")
def basket_locations():
    try:
        sql = f"""
            SELECT
                bl.basket_id,
                ...
                bl.gx,
                bl.gy,
                datetime({NORM.format(col="bl.last_seen")}, 'unixepoch', 'localtime') AS last_seen
            FROM basket_locations AS bl
            LEFT JOIN markers AS m ON m.id = bl.basket_id
            ORDER BY bl.basket_id
        """
        df = query_df(sql)
        if df.empty:
            # 保证表头稳定
            df = df.reindex(columns=["basket_id", "name", "gx", "gy", "last_seen"])
